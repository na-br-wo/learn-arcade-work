...
def on_draw(delta_time):
    ...
    ----------
    delta_time : float
        Controls the rectangle's position
    on_draw.center_x : float
        Variable that controls rectangle's x position
    on_draw.center_y : float
        Variable that controls rectangle's y position
    """
    ...
    # Modify rectangles position based on the delta vector
    # Delta vector is like the speed and direction
    on_draw.center_x += on_draw.delta_x * delta_time
    on_draw.center_y += on_draw.delta_y * delta_time

    # Figure out if we hit the edge and need to reverse. (BOUNCE OFF EDGE)
