...
def print_args(print_args):
    """
    A function that prints the options the user has when running conversion
    """
...
def no_param_mode():
    """
    A function used to inform the user they have not entered any parameters.
    """
    print("Which conversion utility would you like to try?")

def process_args(p_args):
    """
    Passes through input from user to run a variety of conversion modules.

    Parameters
    ----------
    command_list : list
        A list of strings, used to run the print_args() function and show
        which commands the user has.
    p_args : list
        List created as the user passes through commands.
    """
